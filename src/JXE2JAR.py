...
class J9ROMMethod(object):
    ...
    def read(stream):
        ...
        if (len(sys.argv) == 4) and (name == sys.argv[3]):
          print "\nFound " + name + " method in " + iClass + ".class\n" + sys.argv[1] + " offset: " + \
          hex(offset) + " Bytecode: " + binascii.hexlify(bytecode).upper()
...
def create_jar(jar_name, jxe, decompilation_check=True):
    print "Creating output jar file: " + jar_name
...
def process(jxe_name, jar_name):
    print "Reading input file: " + jxe_name
